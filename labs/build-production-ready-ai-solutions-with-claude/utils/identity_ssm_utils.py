...
def get_cognito_client_secret() -> str:
    ...
    response = client.describe_user_pool_client(
        UserPoolId=get_ssm_parameter("/app/returnsrefunds/agentcore/userpool_id"),
        ClientId=get_ssm_parameter("/app/returnsrefunds/agentcore/machine_client_id"),
    )
...
def runtime_resource_cleanup():
    try:
        ...
        for repo in repositories['repositories']:
            if 'bedrock-agentcore-returns_refunds_agent' in repo['repositoryName']:
...
def delete_observability_resources():
    ...
    try:
        # Get agent ARN from parameter store and extract agent name
        agent_arn = get_ssm_parameter("/app/returnsrefunds/agentcore/runtime_arn")
        agent_identifier = agent_arn.split('/')[-1]  # Extract agent-name-unique-id
        agent_name_match = re.match(r'^(.+)-[A-Za-z0-9]+$', agent_identifier)
        
        if not agent_name_match:
            print("  ⚠️  Could not extract agent name from ARN, skipping cleanup")
            return
            
        agent_name = agent_name_match.group(1)
        print(f"  🔍 Cleaning observability resources for agent: {agent_name}")
        
        # Find and delete AgentCore Runtime log groups for this agent
        ...
        for page in paginator.paginate(logGroupNamePrefix='/aws/bedrock-agentcore/runtimes/'):
            for log_group in page['logGroups']:
                ...
                # Check if this log group belongs to our agent
                pattern = rf'^/aws/bedrock-agentcore/runtimes/{re.escape(agent_name)}-[A-Za-z0-9]+-DEFAULT$'
                if re.match(pattern, log_group_name):
                    try:
                        print(f"  🗑️  Deleting log group: {log_group_name}")
                        logs_client.delete_log_group(logGroupName=log_group_name)
                        print(f"  ✅ Successfully deleted: {log_group_name}")
                        deleted_count += 1
                    except Exception as e:
                        if e.response["Error"]["Code"] == "ResourceNotFoundException":
                            print(f"  ℹ️  Log group already deleted: {log_group_name}")
                        else:
                            print(f"  ⚠️  Error deleting {log_group_name}: {e}")
        
        if deleted_count == 0:
            print(f"  ℹ️  No observability resources found for agent: {agent_name}")

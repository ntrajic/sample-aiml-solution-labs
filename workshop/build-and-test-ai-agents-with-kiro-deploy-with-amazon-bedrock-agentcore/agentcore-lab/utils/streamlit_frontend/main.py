...
# Load Cognito configuration from Lab 5 (cognito_config.json)
cognito_config_path = os.path.join(project_root, 'cognito_config.json')
try:
    with open(cognito_config_path, 'r') as f:
        secret = json.load(f)
except FileNotFoundError:
    st.error("❌ cognito_config.json not found. Please run Lab 5 first to set up Cognito.")
    st.stop()
except Exception as e:
    st.error(f"❌ Error loading Cognito configuration: {e}")
    st.stop()


def get_oauth_token(client_id, client_secret, discovery_url):
    """Get OAuth client_credentials token for agent invocation"""
    try:
        ...
        if response.status_code != 200:
            ...
            st.error(f"OAuth token request failed: {response.status_code}")
            st.error(f"Error details: {error_detail}")
            st.info("This Cognito User Pool may not have OAuth configured. Please run Lab 5 to set up Cognito properly.")
            return None
            
        response.raise_for_status()
        return response.json()['access_token']
    except Exception as e:
        st.error(f"Failed to get OAuth token: {e}")
        return None
...
# After user login, get a separate OAuth token for agent invocation
# This is different from the user's access token
if 'oauth_agent_token' not in st.session_state:
    oauth_token = get_oauth_token(
        secret['client_id'],
        secret['client_secret'],
        secret['discovery_url']
    )
    if oauth_token:
        st.session_state['oauth_agent_token'] = oauth_token
    else:
        st.error("Failed to obtain OAuth token for agent access")
        st.stop()
...
st.title("Returns and Refunds Agent")
...
if prompt := st.chat_input("What is up?"):
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            bearer_token = st.session_state.get("oauth_agent_token")
            
            # Show initial thinking state with pulsing animation
            message_placeholder.markdown(f'<span class="thinking-bubble">🤖 💭 Returns and Refunds Agent is thinking...</span>', unsafe_allow_html=True)
        ...
        accumulated_response = chat_manager.invoke_endpoint_nostreaming( agent_arn=st.session_state["agent_arn"],
            payload=payload,
            bearer_token=st.session_state["oauth_agent_token"],
